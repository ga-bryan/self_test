...
class LoggerFactory(object):
    ...
    def set_directory(directory=None, parent_log_dir=None, append_to_parent_log=None, force=False):
        ...
        with LoggerFactory.lock:
            if not directory:
                directory = os.path.join(get_project_base_path, 'logs')
    ...
    def get_schedule_logger(job_id):
        ...
        job_log_dir = os.path.join(get_project_base_path, 'logs', job_id)
        os.makedirs(job_log_dir, exist_ok=True)
        logger = logging.getLogger(job_id)
        logger.setLevel(LoggerFactory.LEVEL)
        handler = LoggerFactory.get_handler(class_name=None, level=LoggerFactory.LEVEL, log_dir=job_log_dir,
                                            is_schedule=True)
        error_handler = LoggerFactory.get_handler(class_name=None, level=logging.ERROR, log_dir=job_log_dir,
                                                  is_schedule=True)
        logger.addHandler(handler)
